...
object HNCrawler {
  ...
  implicit object HNItemReader extends BSONDocumentReader[HNItem] {
    def read(bson: BSONDocument): HNItem =
      new HNItem(
    ...
    case Start =>
      ...
      Future
        .fromTry(
          Try(
            if (!inProgress) true
            else throw new Exception("in progress")
          )
        )
        .onComplete {
          case Success(_) =>
      ...
      http
        ...
        .flatMap {
          case HttpResponse(StatusCodes.OK, _, e, _) =>
            Unmarshal(e).to[List[Int]]
          case HttpResponse(status, _, e, _) =>
            e.discardBytes()
            Future.failed(
              new Exception(s"service returned ${status.intValue()}")
            )
        }
        ...
        case itemID :: rest =>
          ...
          http
            ...
            .onComplete {
              ...
              case Failure(e) =>
                log.error(
                  s"HNCrawler: Get item[${itemID}] error, ${e.getMessage()}"
                )
                self ! Retry
            }
        case Nil =>
